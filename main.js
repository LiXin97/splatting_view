...
async function main() {
    ...
    // Assuming `gl` is your WebGL context and `program` is your shader program
    ...
    document.getElementById('RenderMode').addEventListener('change', (e) => {
        ...
        depthBarLegend.style.display = 'none';
        if (e.target.value === 'depth') {
            depthBarLegend.style.display = 'block';
        } else {
            depthBarLegend.style.display = 'none';
        }
    });

    let GaussianSize = "nomral";
    document.getElementById('GaussianSize').addEventListener('change', (e) => {
        GaussianSize = e.target.value;
    }
    );
    
    // Ensure you update the uniform value in your rendering loop as well
    gl.uniform1i(gl.getUniformLocation(program, 'mode'), renderingMode);

    document.getElementById("near").addEventListener("change", (e) => {
        gl.uniform1f(u_near, e.target.value);
    }
    );
    document.getElementById("far").addEventListener("change", (e) => {
        gl.uniform1f(u_far, e.target.value);
    }
    );

    // Set the near and far clipping plane values
    // Get from html input
    gl.uniform1f(u_near, document.getElementById("near").value);
    gl.uniform1f(u_far, document.getElementById("far").value);
    ...
    const frame = (now) => {
        ...
        if (activeKeys.includes("ControlLeft") || activeKeys.includes("ControlRight"))
            inv = translate4(inv, 0, 0.03, 0);
